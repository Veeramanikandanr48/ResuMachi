...
export const generateAltaCv = (profile: UserProfile): string => {
...
\\begin{document}
\\name{${esc(p.fullName || p.firstName + ' ' + p.lastName)}}
\\tagline{${esc(p.title || 'Professional')}}

\\personalinfo{%
  \\email{${esc(p.email)}}
  \\phone{${esc(p.phone)}}
  ${p.linkedin ? `\\linkedin{${esc(p.linkedin.replace(/^https?:\/\/(www\.)?linkedin\.com\/in\//, ''))}}` : ''}
  ${p.website ? `\\homepage{${esc(p.website.replace(/^https?:\/\//, ''))}}` : ''}
...
\\cvevent{${esc(exp.role)}}{${esc(exp.company)}}{${esc(exp.startDate)} -- ${exp.current ? 'Present' : esc(exp.endDate)}}{${esc(exp.location || 'Remote')}}
\\begin{itemize}
  ${(exp.description || '').split('\\n').filter(l => l.trim()).map(line => `
  \\item ${esc(line.replace(/^[•-]\s*/, ''))}
  `).join('')}
\\end{itemize}
\\divider
`).join('')}
...
\\cvevent{${esc(proj.name)}}{${esc(proj.technologies)}}{}{}
\\begin{itemize}
  \\item ${esc(proj.description)}
\\end{itemize}
\\divider
`).join('')}
`;
...
\\cvevent{${esc(edu.degree || edu.degreeType)} ${esc(edu.field)}}{${esc(edu.institution)}}{${esc(edu.gradMonthYear || edu.graduationDate)}}{${esc(edu.location)}}
\\divider
`).join('')}
`;
  }
...
\\cvtag{${esc(cat.category)}}: 
${cat.items.map(item => `\\cvtag{${esc(item)}}`).join(' ')}
\\par\\divider
`).join('')}
`;
  }
...
`;
  ...
  // For AltaCV, we usually put Experience and Projects in the main column (left),
  // and Education and Skills in the side column (right).
  // The template uses \switchcolumn to move to the right.

  const mainColumn = experience + projects;
  const sideColumn = `
\\switchcolumn
${education}
${skills}
`;
  ...
  return header + mainColumn + sideColumn + footer;
};
